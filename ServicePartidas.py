...
class ServicesPartidas():
    def iniciar_partida(self, nombre, dificultad, palabra, tipo_palabra):
        if(dificultad < 1 or dificultad > 10):
            ...
            return Partida(palabra, intentos_para_partida, tipo_palabra,
                           nombre)
    ...
    def intentar_letra(self, partida, letra):
        ...
        if(letra in partida.palabra):
            '#Usamos el metodo get_letras_position para encontrar todas las'
            '#posiciones'
        ...
        if (partida.intentos <= 0 and partida.palabra_aciertos !=
                partida.palabra):
            return "Perdio"
